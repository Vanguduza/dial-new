...
`DEC-020` upgrades the DIAL Versioned Engineering Knowledge Layer (VEKL) into a horizontal **federated engineering knowledge/capability layer**, not a product Feature fan-out and not a new source of truth. Skills remain one qualified resource class, joined by task-relevant official docs/repos/releases/issues/advisories/package registries, qualified tools/plugins/MCPs, DIAL rules/hooks/loops and bounded community corroboration. Every material Oracle development packet resolves one versioned Engineering Knowledge Activation Manifest after Feature/JIT context. In parallel, exact project-aware Sol→Sonnet ahead-of-work research forecasts and pre-caches the engineering references likely needed by the next 3–5 dependency-safe packets selected by this plan. It cannot alter this plan's priority/dependencies. Canon/FRC/security/current code/evidence remain authoritative and the normal contract/reviewer/gate ladder is unchanged.

The Rev 1 Android qualification wave remains the first executable VEKL slice and is production-qualified for six governed procedures: four exact upstream skills (Adaptive Compose, Navigation 3, Edge-to-edge and Android Intent Security) plus two DIAL-owned wrappers (`dial.android.device-verification` and `dial.android.testing-setup`). All six are exact-pin/exact-hash immutable `ENGINEERING_GUIDANCE_ONLY` inputs. The upstream Android CLI remains quarantined for a blocked curl-pipe-shell install instruction and the upstream Testing Setup skill remains barred from direct activation; the approved DIAL wrappers are the safe routes for those procedures. This does not alter the Track A/Grocery Rounds programme priority. VEKL remains an execution-quality multiplier applied to the packet the plan already chooses, not a competing product workstream.

### 2026-09-08 VEKL 2.1 + technical cohesion

`DEC-024` hardens VEKL resource selection without changing this plan's feature priority. Hard eligibility runs before ranking; the exact-pin Skill resolver is the only Skill-selection owner; optional resources compete only inside deterministic purpose+role slots; complementary roles survive together; registry identity is fingerprinted into packet provenance; and external context defaults to descriptor-only. The result is a smallest qualified engineering coalition rather than a global top-N leaderboard. The same selected identities remain stable under input reorder and Sol→Sonnet failover.

The same decision adds a cross-tool cohesion gate: every cross-cutting concern has one DIAL authority and supporting tools are bounded adapters/projections. PostHog is retained only for product/web analytics, privacy-gated replay/heatmaps, bounded internal/progressive exposure and GMPC-owned experiment assignment/measurement through `@dial/product-telemetry`. Domain events/event taxonomy remain behavioral truth and DIAL activation/certification state remains above flags. PostHog outages cannot fail core transactions and PostHog cannot replace Formbricks, Langfuse, Promptfoo, the OpenTelemetry/Grafana technical plane, Metabase, Command Centre or Temporal/BullMQ/n8n. This is a cohesion change, not a new product workstream, and therefore does not reorder Track A/B/C.

### 2026-09-08 Oracle development-readiness fallback
`DEC-021` separates **development readiness** from full control-plane production certification without weakening the runtime chain. Exact Sol remains primary. If its identity is proven but it is temporarily account/rate/model limited, a fingerprint-bound `DEVELOPMENT_READY_FALLBACK` gate may allow the existing Track A/Track C/Grocery Rounds programme to continue through exact Sonnet 5 on the external Oracle queue after repository verification, a real queue fallback canary, VEKL fallback/research evidence and DIAL-only continuity all pass. This does not reorder this plan, create a third model, or claim `PRODUCTION_GREEN`; the full Sol/Sonnet process/recovery qualification is still required when Sol capacity returns.

### 2026-09-08 Sol capacity preservation
`DEC-022` makes model capacity a governed orchestration resource rather than disposable probe traffic. Sol is still the preferred manager/complex-work runtime, but Oracle now reuses fresh exact-identity/runtime evidence, suppresses inference during provider cooldowns, keeps VEKL forecasts current across volatile mission-turn changes, and routes ordinary work directly to exact Sonnet 5 while a proven Sol capacity boundary is active. This does not change feature priority, manager authority, acceptance gates or the locked Sol → Sonnet → no-runtime chain; it prevents certification/health machinery from consuming the capacity intended for development.
