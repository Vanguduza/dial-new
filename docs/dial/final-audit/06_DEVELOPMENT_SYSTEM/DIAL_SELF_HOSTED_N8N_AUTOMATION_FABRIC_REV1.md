...
## 14. Research instruction and required evidence

n8n research must keep `VEKL_N8N_CORPUS`, `DIAL_N8N_DEV` and `DIAL_N8N_PROD` distinct. It must
report exact node/package provenance and versions, corpus security findings, runtime node-policy
projection, secret and egress posture, authoritative-table boundaries, signed-event handling,
idempotency, retry/dead-letter behaviour and every estate-isolation dimension. Corpus examples
are non-executable reference evidence and never proof that either runtime exists or is qualified.

Runtime claims require a release/content hash, node-policy hash, test evidence, approval and
rollback identity plus an estate descriptor proving credential/store/network separation. When
live deployment evidence is absent the state stays `UNVERIFIED_FROM_REPOSITORY`; research may
not manufacture a runtime result.
