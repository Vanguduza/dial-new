...
Development owner WhatsApp has one fail-closed adapter over the typed tool layer. `whatsapp-hermes-operator.mjs` consumes the Hermes Baileys bridge only through the dedicated Dial Hermes Control bot session with one owner allowlist. The Meta Cloud adapter remains product/reference infrastructure and is not a development owner-control path. `operator-text-router.mjs` implements the typed control grammar and owner-steering contracts.
