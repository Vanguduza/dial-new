...
Changes reach `master` through a pull request with `project-truth` plus every production verification lane (`gates, types and unit suites`, `production build and full typecheck`, `pipeline produces a conforming pack`, and `customer transition contract`) required, owner CODEOWNER review, stale-review dismissal, signed commits, and no force-push or deletion. Project Truth automation is read-only after merge, as required by
