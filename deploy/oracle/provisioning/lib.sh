...
# Recovery provisioning has exactly one mutable target: oracle-admin.
# dial-hermes-control is production control authority and vekl-worker is the
# background coordinator. Neither is provisioned, rebuilt, reconfigured or
# repurposed by this recovery-only tooling.
NEVER_PROVISION=("dial-hermes-control" "vekl-worker")
PROVISIONABLE_HOSTS=("oracle-admin")
