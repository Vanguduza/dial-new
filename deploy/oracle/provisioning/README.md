...
`vm.swappiness=10`. Commander is protected inside `dial-recovery.slice` with
`MemoryMin=192M`, `MemoryHigh=288M`, `MemoryMax=320M`, `CPUWeight=200` and
`OOMScoreAdjust=-500`; SSH and the Oracle agent remain the higher-priority recovery floor.
