...
NODE_ARM64_SHA256="${NODE_ARM64_SHA256:-a44aeb94849a299b22df10b9e622ec2f605c2183501bc40590705131de7c740f}"
NODE_X64_SHA256="${NODE_X64_SHA256:-df450af89261115ef9f9e3830c3eeb2cc9213b63c720b1af623cb5dcbe2e02de}"
ADMIN_USER=ubuntu
...
phase5() {
  ...
  # is intentionally oracle-admin only in the three-node topology; deriving the
  # hostname and validating it against hosts.json prevents accidental role drift.
...
phase6() {
  ...
  # Desktop Commander 0.2.50 rotates its refresh token but does not persist the
  # rotated token. Patch the exact audited version before the supervised service
  # starts so a restart does not fall back to an unattended browser-code flow.
  local session_patch
  session_patch="$(bash "$REPO_DIR/deploy/oracle/provisioning/commander-session-persistence-patch.sh")" || return 1
  fact commander_session_persistence "$session_patch"
  ...
  local rt=/run/user/$(id -u $ADMIN_USER)
  # Normalize stale runtime resource overrides created by earlier
  # `systemctl set-property --runtime` experiments. These live under user.control
  # and override the repository unit even after that unit is repaired. Remove only
  # this service's transient property drop-ins so effective runtime values converge
  # on the durable unit (MemoryHigh=288M, MemoryMax=320M, CPUWeight=200).
  rm -rf "$rt/systemd/user.control/dial-commander-remote.service.d" 2>/dev/null || true
  fact commander_runtime_property_drift_reset true
  sudo -u $ADMIN_USER XDG_RUNTIME_DIR=$rt systemctl --user daemon-reload || true
