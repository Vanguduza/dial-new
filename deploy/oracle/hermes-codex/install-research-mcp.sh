...
for command in node npm install sha256sum; do command -v "$command" >/dev/null || { echo "Missing required command: $command" >&2; exit 2; }; done
...
const expected={'@upstash/context7-mcp':'4.1.1','exa-mcp-server':'3.4.1'};
...
"$BIN_DIR/dial-exa-mcp" --version | grep -Fx '3.4.1' >/dev/null
printf 'Installed DIAL research MCP runtime: Context7 4.1.1 + Exa 3.4.1\n'
