...
jq -e '.execution_origin=="EXTERNAL_DIAL_ORCHESTRATOR" and .result.event=="HERMES_OPERATIONAL_TURN_COMPLETED" and .result.runtime=="claude_code" and .result.requested_model=="claude-sonnet-5" and .result.resolved_model=="claude-sonnet-5" and .result.fallback_used==true' "$CANARY" >/dev/null || { cat "$CANARY" >&2; fail "external queue did not execute through exact Sonnet fallback"; }
pass "external DIAL queue executes a real fallback canary through exact Sonnet"
...
jq -n \
  ...
  '{schema_version:3,status:"DEVELOPMENT_READY_FALLBACK",development_only:true,production_certified:false,execution_origin:"EXTERNAL_DIAL_ORCHESTRATOR",runtime_policy:"gpt-5.6-sol -> claude-sonnet-5 -> NO_HERMES_RUNTIME_AVAILABLE",development_entrypoint:"dial-hermes-submit",direct_project_session_development_allowed:false,qualified_repo_head:$repo_head,control_plane_fingerprint:$control_plane_fingerprint,dial_doctor:{federated:true,hermes_native_subordinate:true,authority:"DIAGNOSTIC_EVIDENCE_ONLY",status:$hermes_native_doctor_status,report_sha256:$hermes_native_doctor_hash},primary:{runtime:"codex_app_server",requested_model:"gpt-5.6-sol",resolved_model:"gpt-5.6-sol",identity_proven:true,state:$primary_state},fallback:{runtime:"claude_code",requested_model:"claude-sonnet-5",resolved_model:"claude-sonnet-5",identity_proven:true,state:"HEALTHY"},external_fallback_canary:{completed:true,execution_origin:"EXTERNAL_DIAL_ORCHESTRATOR",job_id:$canary_job_id,resolved_model:"claude-sonnet-5"},vekl:{policy_version:"vekl-2.1",live_fallback_canary:true,activation_id:$vekl_activation,evidence:$vekl_evidence,exact_hashes_preserved:true,federated_resource_provenance_preserved:true,ahead_of_work_forecast_ready:true,forecast_id:$forecast_id,forecast_runtime:"claude_code",forecast_resolved_model:"claude-sonnet-5"},continuity:{green:true,evidence:$continuity_evidence},repository_verification:{green:true},upgrade_required:{target:"PRODUCTION_GREEN",when:"exact GPT-5.6 Sol becomes healthy again",steps:["bash deploy/oracle/hermes-codex/qualify-control-plane.sh","bash deploy/oracle/hermes-codex/soak-control-plane.sh process","bash deploy/oracle/hermes-codex/soak-external-orchestrator.sh","bash deploy/oracle/hermes-codex/soak-control-plane.sh continuity","bash deploy/oracle/hermes-codex/finalize-control-plane.sh"]},observed_at:$observed_at}' >"$EVIDENCE"
chmod 600 "$EVIDENCE"
