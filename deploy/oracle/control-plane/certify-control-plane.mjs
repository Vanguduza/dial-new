...
assert.equal(allowedOnHost('vekl-worker', 'VEKL_LIGHT'), true);
assert.equal(allowedOnHost('vekl-worker', 'HEAVY_BUILD'), false);
assert.equal(allowedOnHost('dial-hermes-control', 'HEAVY_BUILD'), false);
assert.equal(allowedOnHost('dial-hermes-control', 'HERMES_RUNTIME'), true);
const heavy = route({ workload_class: 'HEAVY_BUILD', target_host: 'oracle-admin' }, { localHost: 'oracle-admin' });
assert.equal(heavy.decision, 'REFUSE');
assert.equal(heavy.reason, 'NO_ELIGIBLE_HOST');
assert.equal(route({ workload_class: 'VEKL_LIGHT' }, { localHost: 'oracle-admin' }).target_host, 'vekl-worker');
...
assert.equal(planSemanticOperation({ operation: 'HOST_HEALTH', target_host: 'vekl-worker' }).decision, 'ALLOW');
assert.equal(planSemanticOperation({ operation: 'HOST_HEALTH', target_host: 'vekl-worker' }).transport, 'DIRECT_SSH');
assert.equal(planSemanticOperation({ operation: 'SERVICE_STATUS', target_host: 'oracle-admin', args: { service: '../bad' } }).decision, 'REFUSE');
console.log(JSON.stringify({ certification: 'GREEN', topology: 'THREE_NODE_ACTIVE', development_via_recovery_plane: false, assertions: 18 }, null, 2));
