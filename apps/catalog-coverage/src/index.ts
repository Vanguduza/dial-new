...
const DEFAULT_CATALOG = '../nissan gtr auto/Nissan-Gtr-Auto/data-pipeline/data/dist/gtr_catalog_7zap_v1.sqlite';
...
function deriveFamilyName(displayName: string): string {
  const cleaned = displayName
    .replace(/\s+(?:first|second|third|fourth|fifth|sixth|seventh|eighth|ninth|tenth|\d+(?:st|nd|rd|th))\s+generation.*$/i, '')
...
async function build(): Promise<void> {
  ...
  const meta = Object.fromEntries(
    (db.prepare('SELECT key, value FROM catalog_meta').all() as Array<{ key: string; value: string }>).map((row) => [row.key, row.value]),
  );
  const makers = db.prepare('SELECT slug, name, source FROM makers ORDER BY lower(name), slug').all() as unknown as MakerRow[];
  ...
  const sectionCounts = metricMap(db.prepare('SELECT maker_slug, model_slug, COUNT(*) AS count FROM sections GROUP BY maker_slug, model_slug').all() as unknown as MetricRow[]);
  ...
  const diagramImageCounts = new Map(diagramRows.map((row) => [`${row.maker_slug}/${row.model_slug}`, Number(row.image_count)]));
  const partCounts = metricMap(db.prepare('SELECT maker_slug, model_slug, COUNT(*) AS count FROM diagram_parts GROUP BY maker_slug, model_slug').all() as unknown as MetricRow[]);
  ...
  const integrity: CatalogIntegrityGate = {
    ...
    notes: Number(collision.count) === 0
  ...
  const developmentFlowPacks: FlowPackReference[] = [
    {
      ...
      scopeNote: 'Complete development experience; production publication remains blocked on licensed assets and corrected catalog identities.',
    },
  ];
  ...
  const observedMakers = makers.map((maker) => ({
    ...
    models: (modelsByMaker.get(maker.slug) ?? []).map((model) => {
      ...
      const flowPack = developmentFlowPacks.find((pack) => pack.makerSlug === maker.slug && pack.modelSlug === model.slug) ?? null;
      ...
      return {
        ...
        variant: {
          ...
          identityQuality: model.variant_slug === model.slug ? 'SYNTHETIC_MODEL_DUPLICATE' : 'SOURCE_VARIANT',
        },
        metrics,
        flowPack,
        stages,
  ...
  const universeMakerSlugs = [...new Set([...makers.map((maker) => maker.slug), ...supplemental.makers.map((maker) => maker.slug)])];
  ...
  const summary = {
    ...
    counts: {
      ...
      customerReadyModels: observedMakers.flatMap((maker) => maker.models).filter((model) => model.customerVisible).length,
  ...
  const csvRows: Array<Array<string | number | boolean>> = [[
    'maker_slug', 'maker_name', 'model_slug', 'model_name', 'family_name', 'source_state',
    'priority', 'customer_visible', 'section_count', 'diagram_count', 'part_row_count', 'first_blocking_stage',
  ]];
  for (const maker of observedMakers) {
    for (const model of maker.models) {
      csvRows.push([
        maker.slug, maker.name, model.slug, model.name, model.familyName, model.sourceState, '',
        model.customerVisible, model.metrics.sectionCount, model.metrics.diagramCount, model.metrics.partRowCount,
  ...
  for (const model of supplementalModels) {
    csvRows.push([
      model.makerSlug, model.makerName, model.slug, model.name, model.familyName, model.sourceState,
      model.priority, false, 0, 0, 0, model.evaluation.firstBlockingStage ?? '',
    ]);
  }
  await writeFile(
...
async function check(): Promise<void> {
  const outputRoot = resolve(option('--output', DEFAULT_OUTPUT));
  const ledger = JSON.parse(await readFile(resolve(outputRoot, 'catalog-coverage-ledger.json'), 'utf8')) as {
    observedMakers: Array<{ slug: string; models: Array<{ customerVisible: boolean; evaluation: { customerReady: boolean } }> }>;
    acquisitionBacklog: Array<{ makerSlug: string; customerVisible: boolean; evaluation: { customerReady: boolean } }>;
  ...
  for (const maker of requiredMakers) {
    if (!ledger.observedMakers.some((entry) => entry.slug === maker)) throw new Error(`Observed catalog is missing ${maker}`);
  }
  for (const maker of ['isuzu', 'nissan', 'chery', 'gwm', 'haval', 'byd', 'geely', 'omoda', 'jaecoo']) {
    if (!ledger.acquisitionBacklog.some((entry) => entry.makerSlug === maker)) throw new Error(`Acquisition backlog is missing ${maker}`);
  }
  const leaked = [...observedModels, ...ledger.acquisitionBacklog].filter((model) => model.customerVisible && !model.evaluation.customerReady);
  if (leaked.length) throw new Error(`${leaked.length} incomplete vehicle(s) leaked into the customer-visible catalog`);
  if (!ledger.developmentFlowPacks.length) throw new Error('No complete development flow pack is registered');
  process.stdout.write(`Coverage check passed: ${observedModels.length} observed models, ${ledger.acquisitionBacklog.length} acquisition targets, ${ledger.developmentFlowPacks.length} development flow pack(s).\n`);
}
