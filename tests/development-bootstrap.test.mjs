...
describe('DIAL development bootstrap closure', () => {
  ...
  it('keeps Dial Control zero-touch bootstrap complete, bounded and resumable', () => {
    ...
    expect(fs.readFileSync(path.join(repoDir, 'deploy/netcup/hermes-control/migrate-from-oracle-control.sh'), 'utf8')).toContain(`[[ "$rel" == .hermes ]] && excludes=(--exclude '/bin/' --exclude '/hermes-agent/')`);
