...
describe('DIAL development bootstrap closure', () => {
  ...
  it('pins every mandatory supply-chain path and binds auxiliary lock/vendor artifacts', () => {
    ...
    expect(Object.entries(pins.pins).filter(([, pin]) => pin?.pin_state && !String(pin.pin_state).startsWith('PINNED'))).toEqual([]);
        expect(pins.pins.artemis.pin_state).toBe('PINNED_WITH_MANDATORY_LOCAL_HARDENING');
        expect(pins.pins.uv.pin_state).toBe('PINNED_ATTESTED_RELEASE');
