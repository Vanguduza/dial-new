...
const roots: string[] = [];
const source =
  '<svg xmlns="http://www.w3.org/2000/svg" width="320" height="180"><rect width="320" height="180" fill="#0a0e14"/><path d="M48 126 L67 86 L110 75 L137 50 L210 50 L238 80 L277 98 L285 126 Z" fill="#c8d2dc"/><circle cx="92" cy="130" r="17" fill="#e8eef4"/><circle cx="241" cy="130" r="17" fill="#e8eef4"/></svg>';

async function fixture(commercialUseApproved = true) {
  const root = await mkdtemp(join(tmpdir(), "dvtg-test-"));
  roots.push(root);
  const sourcePath = join(root, "source.svg");
  await writeFile(sourcePath, source);
  const job = {
    visualFamilyId: "VF-TEST-PICKUP-DC",
    make: "Test",
    model: "Pickup",
    generation: "One",
    bodyStyle: "Double Cab",
    visualPhase: "Original",
    yearFrom: 2026,
    yearTo: null,
    ...
    provenance: {
      frontThreeQuarter: {
        ...
        author: "Test",
        licence: "Original test fixture",
        licenceUrl: "test://licence",
        attribution: "Test",
    ...
    fitmentMapping: {
      schemaVersion: "2.0.0",
      mappingId: "VEM-TEST-PICKUP-ONE",
      catalogReleaseId: "CAT-TEST-V2",
      fitmentId: "FIT-TEST-ONE",
      visualFamilyId: "VF-TEST-PICKUP-DC",
      vehicleContext: {
        makerSlug: "test",
        catalogFamilyId: "CF-TEST-PICKUP",
        familySlug: "test-pickup",
        variantId: "CV-TEST-PICKUP-ONE",
        variantSlug: "test-pickup-one",
        chassisCodes: ["TEST-1"],
        engineCodes: [],
        market: null,
        attributes: { bodyStyle: "double-cab" },
      },
      categories: [
        {
          visualCategoryId: "VC-ENG",
          componentFamilyId: "VCF-TEST-ENGINE",
          label: "Engine",
          target: {
            sectionSlug: "engine",
            groupId: "GRP-TEST-ENGINE",
            groupSlug: "engine",
            defaultDiagramId: "DGM-TEST-ENG-001",
        ...
        },
        {
          ...
          componentFamilyId: "VCF-TEST-TRANSMISSION",
          label: "Transmission",
          target: {
            sectionSlug: "transmission-drivetrain",
            groupId: "GRP-TEST-TRANSMISSION",
            groupSlug: "transmission",
            defaultDiagramId: "DGM-TEST-TRN-001",
            fallbackQuery: "transmission",
            selectionMode: "GROUP",
            minimumReadiness: "BROWSE_READY",
          },
        },
        {
          ...
          componentFamilyId: "VCF-TEST-BODY",
          label: "Body",
          target: {
    ...
    motionProfile: "test-v1",
    generationProvider: "deterministic-development",
  };
...
describe("pipeline integration", () => {
  it("blocks an unapproved source before processing", async () => {
    await expect(validateJobFile(await fixture(false))).rejects.toThrow(/commercialUseApproved/);
  });
  ...
  it("runs every stage and resumes unchanged successful work idempotently", async () => {
    ...
    const flowPack = JSON.parse(
      await readFile(join(second.packRoot, "navigation", "hero-to-epc-flow-pack.json"), "utf8"),
    );
    expect(flowPack.customerReady).toBe(false);
    expect(flowPack.entryModes).toEqual(
    ...
    expect(flowPack.autoplay).toMatchObject({
      trigger: "VEHICLE_SEARCH_COMMITTED",
      userPlayControl: false,
    });
    expect(flowPack.retainedSelection).toMatchObject({
      preserveUntilEdited: true,
      committedTextAppearance: "FAINT_GREY",
    });
    expect(flowPack.stages.map((stage: { id: string }) => stage.id)).toEqual(
      expect.arrayContaining(["HERO_PHOTOGRAPHY", "EXPLODED_SYSTEMS", "EPC_DIAGRAM_AND_PARTS"]),
    );
    expect(flowPack.stages.map((stage: { id: string }) => stage.id)).not.toContain(
      "TECHNICAL_SHADED",
    );
