...
// Provider routing evolves rather than being replaced (Rev 3.1 §6.9). The legacy
// signature is preserved exactly: when no guided inputs are supplied, the result
// is byte-identical to the pre-guided router, including its selection_hash.
//
// Routing stays fail-closed. A provider becomes eligible by declaring capability
// AND passing qualification AND being healthy — never by being the only one left.
export const DESIGN_ITERATION_PHASES = Object.freeze(['EXPLORE', 'CONVERGE', 'RECONSTRUCT']);
const PREFERENCE = Object.freeze(['LOCKED_BASELINE_REPAIR', 'DIRECT_DONOR_PORT_AND_TRANSFORM', 'DIRECT_DIAL_IMPLEMENTATION', 'STITCH_CODE_TO_DESIGN_THEN_BUILD', 'STITCH_NEW_DESIGN_THEN_BUILD']);

export function selectDesignStrategy({ designMode, stitchEnabled = false, stitchEligible = false, directWorkerEligible = true, providerHealth = 'UNKNOWN' } = {}) {
  const candidates = [];
  ...
  if (stitchEnabled && stitchEligible && providerHealth === 'HEALTHY') candidates.push(designMode === 'NEW_DIAL_DESIGN' ? 'STITCH_NEW_DESIGN_THEN_BUILD' : 'STITCH_CODE_TO_DESIGN_THEN_BUILD');
  if (!candidates.length) return { ok: false, reason: 'NO_ELIGIBLE_DESIGN_STRATEGY' };
  candidates.sort((a, b) => PREFERENCE.indexOf(a) - PREFERENCE.indexOf(b));
  const selected = candidates[0];
  return { ok: true, candidates, selected, selection_hash: hashObject({ designMode, candidates, selected, providerHealth }) };
}
