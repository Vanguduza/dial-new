...
import { browserFabricHealth } from './browser-acquisition-client.mjs';
...
export class VeklResearchMonitor {
  ...
  async snapshot() {
    ...
    const routeRows=(await db.query(`
      WITH routes AS (
        SELECT COALESCE(payload->>'acquisition_method',payload->>'adapter') AS method,
               COALESCE(payload->>'acquisition_state','PRIMARY_ROUTE_OK') AS route_state
        FROM vekl_research_events ev
        WHERE ev.event_kind IN ('SEARCH','FETCH_READ')
          AND EXISTS (
            SELECT 1 FROM vekl_research_packets p
            WHERE p.mission_id=$1
              AND (p.lease_id=ev.lease_id OR p.packet_id=ev.payload->>'packet_id')
          )
        UNION ALL
        SELECT source->>'acquisition_method' AS method, 'PERSISTED_EVIDENCE' AS route_state
        FROM vekl_research_evidence e
        JOIN vekl_research_packets p ON p.packet_hash=e.packet_hash
        CROSS JOIN LATERAL jsonb_array_elements(CASE WHEN jsonb_typeof(e.sources)='array' THEN e.sources ELSE '[]'::jsonb END) AS source
        WHERE p.mission_id=$1 AND source ? 'acquisition_method'
      )
      SELECT method,route_state,count(*)::int AS count
      FROM routes
      WHERE method IS NOT NULL
      GROUP BY method,route_state
      ORDER BY method,route_state
    `,[mid])).rows;
    const routeCounts={};
    let degradedRoutes=0;
    for(const row of routeRows){
      routeCounts[row.method]=(routeCounts[row.method]||0)+row.count;
      if(row.route_state==='DEGRADED_ROUTE_USED') degradedRoutes+=row.count;
    }
    const browserHealth=await browserFabricHealth();
    ...
    return {
      ...
      acquisition:{
        routes:routeCounts,
        degraded_route_count:degradedRoutes,
        browser_fabric:browserHealth,
      },
  ...
  async packet({ packet_id=null, ordinal=null }={}) {
    ...
    const events=(await this.pool.query(`SELECT event_id,lease_id,event_kind,payload,created_at
      FROM vekl_research_events
      WHERE payload->>'packet_id'=$1
         OR lease_id IN (SELECT DISTINCT lease_id FROM vekl_research_evidence WHERE packet_hash=$2 AND lease_id IS NOT NULL)
      ORDER BY event_id`,[p.packet_id,p.packet_hash])).rows;
  ...
  async alerts() {
    ...
    const rows=(await this.pool.query(`SELECT p.packet_id,p.ordinal FROM vekl_research_packets p
      WHERE p.mission_id=(SELECT mission_id FROM vekl_research_missions WHERE state='READY' ORDER BY updated_at DESC LIMIT 1)
        AND EXISTS(SELECT 1 FROM vekl_research_evidence e WHERE e.packet_hash=p.packet_hash AND e.evidence_kind='ANALYSIS')
      ...
      ORDER BY p.ordinal LIMIT 50`)).rows;
