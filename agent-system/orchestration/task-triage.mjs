...
export const TASK_ARCHETYPES=Object.freeze(['ROUTINE_CODE_CHANGE','BUG_LOCALIZATION','ARCHITECTURE_CHANGE','MONEY_PATH_CHANGE','AUTHORIZATION_CHANGE','EXTERNAL_REFERENCE_RESEARCH','LOCKED_DESIGN_ENHANCEMENT','VISUAL_REGRESSION','NEW_FRONTEND_DESIGN','DATABASE_MIGRATION','SECURITY_REVIEW','INFRASTRUCTURE_CHANGE','DEPENDENCY_UPGRADE','TEST_REPAIR','EVIDENCE_RECONCILIATION','RESEARCH','DOCUMENTATION','AMBIGUOUS']);
...
const EXTREF=/\b(donor|external repositor|reference repo|fixitnow|assimilation|port[- ]wholesale)\b/i;
...
export function classifyTask({unitMap={},instruction='',affectedPaths=[],featureRecord={},contractRecord={},designMode=null}={}){
 ...
 else if(UI.test(text)&&(unitMap?.product_experience_map?.applicable||unitMap?.product_experience_map?.state==='RESOLVED_FOR_KNOWLEDGE'||['REFERENCE_INSPIRED_DIAL_NATIVE','DONOR_ADAPT','DONOR_PRESERVE'].includes(designMode))){archetype='NEW_FRONTEND_DESIGN';risk='MEDIUM';fired.push('PRODUCT_EXPERIENCE');if(EXTREF.test(text)||['REFERENCE_INSPIRED_DIAL_NATIVE','DONOR_ADAPT','DONOR_PRESERVE'].includes(designMode))fired.push('EXTERNAL_REFERENCE_INSPIRATION_ONLY');}
 else if(EXTREF.test(text)){archetype='EXTERNAL_REFERENCE_RESEARCH';risk='LOW';fired.push('EXTERNAL_REFERENCE_RESEARCH_ONLY');}
 ...
 const mandatoryIndependentReview=['HIGH','CRITICAL'].includes(risk)||['NEW_FRONTEND_DESIGN','LOCKED_DESIGN_ENHANCEMENT'].includes(archetype);
 const mandatoryCompetitiveReview=risk==='CRITICAL'&&archetype==='ARCHITECTURE_CHANGE';
 const requiredCapabilities=archetype==='EXTERNAL_REFERENCE_RESEARCH'?['research']:['coding'];if(['NEW_FRONTEND_DESIGN','LOCKED_DESIGN_ENHANCEMENT','VISUAL_REGRESSION'].includes(archetype)) requiredCapabilities.push('visualReasoning');
 const fp=unitMap?.product_experience_map?.frontend_projection;const frontendApplicable=Boolean(fp?.applicable)&&['NEW_FRONTEND_DESIGN','LOCKED_DESIGN_ENHANCEMENT','VISUAL_REGRESSION'].includes(archetype);
