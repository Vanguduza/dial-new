...
function workerServiceState() {
  try { return String(execFileSync('systemctl', ['--user', 'is-active', WORKER_SERVICE], { encoding: 'utf8', timeout: 2500 })).trim() || 'unknown'; }
...
function recentFactoryEvents(limit = 14) {
  try {
    ...
    return currentEpoch.slice(-limit).reverse();
  } catch { return []; }
}
...
<section class="topbar"><div><div class="eyebrow">Dial Health · Design Control Plane</div><h1>Screen Factory</h1><div class="subtitle">Evidence-backed screen generation with functional requirements delivered by Hermes, independent Dial Health design authority, deterministic rendering, live QA and platform-level packaging.</div><div class="statusline"><span id="liveBadge" class="badge"><span class="dot"></span><span>Connecting…</span></span><span id="workerBadge" class="badge"><span class="dot"></span><span>Worker…</span></span><span class="badge"><span class="dot" style="background:#2563eb"></span><span>Hermes: functions only</span></span><span class="badge"><span class="dot" style="background:#8b5cf6"></span><span>Visual authority: independent</span></span></div></div><div class="controlbox"><button class="play" data-act="play">▶ Play</button><button data-act="pause">Ⅱ Pause</button><button data-act="resume">↻ Resume</button><button class="danger" data-act="stop">■ Stop</button></div></section>
...
function statusClass(state){state=String(state||'').toUpperCase();if(['FAILED','RUNTIME_BLOCKED'].includes(state))return'fail';if(['CONTRACT_BLOCKED','PAUSED'].includes(state))return'warn';if(['RUNNING','GENERATING','RENDERING','QA','IMPORTING','WAITING_FOR_CHATGPT'].includes(state))return'live';return'stop'}
...
function syncControls(s){const requested=String(s.requested_state||'STOPPED');document.querySelector('[data-act="play"]').disabled=requested==='RUNNING';document.querySelector('[data-act="pause"]').disabled=requested!=='RUNNING';document.querySelector('[data-act="resume"]').disabled=requested!=='PAUSED';document.querySelector('[data-act="stop"]').disabled=requested==='STOPPED'}
...
function renderEvents(events){$('eventList').innerHTML=(events||[]).map(e=>'<div class="event"><span class="eventdot"></span><div><div class="eventname">'+esc(e.event||'Factory event')+'</div><div class="eventmeta">'+esc(e.task_id||e.batch_id||e.screen_id||e.reason||'control-plane update')+'</div></div><div class="eventtime">'+esc(fmtTime(e.at||e.observed_at))+'</div></div>').join('')||'<div class="subtitle">No recent factory events.</div>'}
...
async function refresh(){if(refreshing)return;refreshing=true;try{const snap=await reqJson('/api/dashboard');latestSnap=snap;const s=snap.status||{},cb=snap.current_batch||{},platforms=snap.platform_progress||[];const cls=statusClass(s.state);$('liveBadge').className='badge '+cls;$('liveBadge').innerHTML='<span class="dot"></span><span>Factory '+esc(s.state)+'</span>';$('workerBadge').className='badge '+(snap.worker_service==='active'?'live':'stop');$('workerBadge').innerHTML='<span class="dot"></span><span>Worker '+esc(snap.worker_service||'unknown')+'</span>';$('lastUpdated').textContent='Updated '+fmtTime(snap.server_time);$('kpis').innerHTML=kpis(s);$('overallText').textContent=s.complete+' / '+s.required_total+' · '+s.percent+'%';$('overallFill').style.width=pct(s.complete,s.required_total)+'%';$('contractText').textContent=s.contract_ready+' / '+s.required_total+' · '+pct(s.contract_ready,s.required_total).toFixed(1)+'%';$('contractFill').style.width=pct(s.contract_ready,s.required_total)+'%';$('mission').innerHTML=renderMission(s,cb);syncControls(s);renderPing(snap.current_ping);renderProgressLists(s,platforms);renderPlatforms(platforms,snap.platform_packages||[]);renderPackages(snap.platform_packages||[]);renderEvents(snap.recent_events||[]);renderGallery(snap.recent_tasks||[],s)}catch(e){$('liveBadge').className='badge fail';$('liveBadge').innerHTML='<span class="dot"></span><span>Connection error</span>';notice('Live refresh failed: '+e.message,'err')}finally{refreshing=false}}
