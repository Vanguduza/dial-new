...
function renderGoogleBootGate(oauth){const o=oauth||{};const gate=$('googleBootGate');if(o.authenticated){gate.classList.add('hidden');$('revokeGoogle').classList.remove('hidden');return}gate.classList.remove('hidden');$('revokeGoogle').classList.add('hidden');const setup=o.state==='SETUP_REQUIRED';$('googleClientSetup').classList.toggle('hidden',!setup);$('googleConnectActions').classList.toggle('hidden',setup);$('googleGateCopy').innerHTML=setup?'One-time administrator setup is required before Google can issue a persistent offline credential. Create a <b>Web application</b> OAuth client in Google Auth Platform, register the callback below, then save the client here.':'Authorize <b>'+esc(o.account_email||'tapiwaguduza@gmail.com')+'</b> once. After consent, the refresh credential stays on Oracle and the factory can archive while your phone is offline.'}
$('saveGoogleClient').addEventListener('click',async()=>{const id=$('googleClientId').value.trim(),secret=$('googleClientSecret').value.trim();if(!id||!secret)return gateError('Enter both the Google OAuth client ID and client secret.');const b=$('saveGoogleClient');b.disabled=true;gateError();try{await reqJson('/api/storage/google-auth/client',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({client_id:id,client_secret:secret})});$('googleClientSecret').value='';notice('Google OAuth client saved securely on Oracle');await refresh()}catch(e){gateError('OAuth client setup failed: '+e.message)}finally{b.disabled=false}});
...
$('editGoogleClient').addEventListener('click',()=>{$('googleClientSetup').classList.remove('hidden');$('googleConnectActions').classList.add('hidden');$('googleGateCopy').innerHTML='Replace the stored OAuth credential pair using the <b>client ID and current client secret from the same Google Web application</b>. Saving here overwrites the stale pair on Oracle.';gateError();});
...
async function completeGoogleCallback(){const q=new URLSearchParams(location.search),code=q.get('google_oauth_code'),state=q.get('google_oauth_state'),error=q.get('google_oauth_error');if(error){gateError('Google authorization failed: '+error);history.replaceState(null,'',location.pathname);return}if(!code||!state)return;try{gateError();$('googleGateCopy').textContent='Completing Google authorization on Oracle…';await reqJson('/api/storage/google-auth/exchange',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({code,state})});history.replaceState(null,'',location.pathname);notice('Google Drive connected for persistent offline archive access');await refresh()}catch(e){const msg=String(e.message||e);gateError('Google authorization could not be completed: '+msg);if(/client secret is invalid|invalid_client/i.test(msg)){$('googleClientSetup').classList.remove('hidden');$('googleConnectActions').classList.add('hidden');$('googleGateCopy').innerHTML='Oracle reached Google, but Google rejected the stored OAuth credential pair. Re-enter the <b>client ID and current client secret from the same Web application</b>, save them, then authorize again.';}history.replaceState(null,'',location.pathname)}}
