...
import { beginGoogleOauth, configureGoogleOauthClient, exchangeGoogleOauth, googleOauthStatus, revokeGoogleOauth, GOOGLE_DRIVE_REDIRECT_URI } from './screen-factory-google-oauth.mjs';
...
function taskSummary(task) {
  return {
    ...
    generator_model: task.generator_provenance?.model || null,
    visual_standard: task.generator_provenance?.visual_standard || null,
    quality_score: task.quality_review?.final_review?.score ?? task.quality_review?.initial_review?.score ?? null,
    quality_repaired: task.quality_review?.repaired === true,
...
async function handleApi(req, res, url) {
  if (req.method === 'GET' && url.pathname === '/api/storage/google-auth/status') return json(res, googleOauthStatus());
  if (req.method === 'POST' && url.pathname === '/api/storage/google-auth/client') {
    try {
      const body = await readJsonBody(req);
      return json(res, { ok: true, google_oauth: configureGoogleOauthClient({ clientId: body.client_id, clientSecret: body.client_secret }) });
    } catch (error) { return json(res, { ok: false, error: String(error?.message || error) }, 400); }
  }
  if (req.method === 'GET' && url.pathname === '/api/storage/google-auth/start') {
    try {
      const started = beginGoogleOauth({ forceConsent: url.searchParams.get('force') === '1' });
      if (url.searchParams.get('redirect') === '1') { res.writeHead(302, { location: started.authorization_url, 'cache-control': 'no-store' }); return res.end(); }
      return json(res, started);
    } catch (error) { return json(res, { ok: false, error: String(error?.message || error), redirect_uri: GOOGLE_DRIVE_REDIRECT_URI }, 409); }
  }
  if (req.method === 'POST' && url.pathname === '/api/storage/google-auth/exchange') {
    try {
      const body = await readJsonBody(req);
      return json(res, { ok: true, google_oauth: await exchangeGoogleOauth({ code: body.code, state: body.state }) });
    } catch (error) { return json(res, { ok: false, error: String(error?.message || error) }, 400); }
  }
  if (req.method === 'POST' && url.pathname === '/api/storage/google-auth/revoke') {
    try { return json(res, { ok: true, google_oauth: await revokeGoogleOauth() }); }
    catch (error) { return json(res, { ok: false, error: String(error?.message || error) }, 400); }
  }
  ...
  if (req.method === 'GET' && url.pathname === '/api/dashboard') {
    ...
    return json(res, { server_time: new Date().toISOString(), status, google_oauth: googleOauthStatus(), worker_service: workerServiceState(), batches: m.batches || [], platform_packages: m.platform_packages || [], platform_progress: platformProgress(m), current_batch, current_ping: request ? { ping_type: request.ping_type, batch_id: request.batch_id, business_unit: request.business_unit, platform: request.platform, expected_count: request.expected_count, hermes_role: request.hermes_role, design_boundary: request.design_boundary, calibration_profile: request.calibration_profile, functional_message: request.functional_message, tasks: request.tasks, prepared_at: request.prepared_at } : null, recent_events: recentFactoryEvents(), recent_tasks });
  }
  ...
  if (req.method === 'GET' && url.pathname === '/api/ping') {
    ...
    return json(res, {
      ...
      expected_count: request.expected_count, hermes_role: request.hermes_role, design_boundary: request.design_boundary, calibration_profile: request.calibration_profile,
...
.gallery{display:grid;grid-template-columns:repeat(auto-fill,minmax(220px,1fr));gap:11px;margin-top:12px;align-items:start}.thumb{background:#fff;border:1px solid var(--line);border-radius:15px;overflow:hidden}.thumb img{width:100%;height:auto;object-fit:contain;background:#f1f5f9;display:block}.empty-visual{aspect-ratio:16/10;background:#f1f5f9;display:grid;place-items:center;padding:14px;text-align:center;color:#64748b;font-size:11px}.meta{padding:10px 11px;font-size:11px;line-height:1.45}.meta b{font-size:12px}.footer-note{margin-top:16px;text-align:center;color:#8292a3;font-size:10px}.hidden{display:none!important}
.authgate{position:fixed;inset:0;z-index:9999;display:grid;place-items:center;padding:18px;background:rgba(12,28,44,.68);backdrop-filter:blur(10px)}.authcard{width:min(560px,100%);max-height:92vh;overflow:auto;background:#fff;border:1px solid #dbe5ea;border-radius:26px;padding:24px;box-shadow:0 24px 80px rgba(8,25,42,.28)}.authbrand{font-size:11px;text-transform:uppercase;letter-spacing:.14em;font-weight:900;color:var(--teal)}.authtitle{font-size:26px;font-weight:900;letter-spacing:-.025em;margin:7px 0}.authcopy{font-size:13px;line-height:1.55;color:var(--muted)}.authfield{width:100%;margin-top:9px;border:1px solid #cbd8df;border-radius:12px;padding:11px 12px;font:inherit}.authcallback{margin-top:10px;padding:10px;border-radius:12px;background:#f1f5f9;font:11px/1.45 ui-monospace,SFMono-Regular,Menlo,monospace;overflow-wrap:anywhere}.authactions{display:flex;gap:8px;flex-wrap:wrap;margin-top:14px}.authnote{margin-top:12px;font-size:11px;line-height:1.5;color:#64748b}
...
</style></head><body><div id="googleBootGate" class="authgate"><div class="authcard"><div class="authbrand">Dial Health · Secure archive</div><div class="authtitle">Connect Google Drive</div><div id="googleGateCopy" class="authcopy">Checking persistent Google Drive authorization on Oracle…</div><div id="googleClientSetup" class="hidden"><div class="authcallback">Authorized redirect URI:<br><b>https://vanguduza.github.io/dial-health-screen-factory-dashboard/oauth/google/callback.html</b></div><input id="googleClientId" class="authfield" autocomplete="off" placeholder="Google OAuth Web client ID"><input id="googleClientSecret" class="authfield" type="password" autocomplete="new-password" placeholder="Google OAuth client secret"><div class="authactions"><button id="saveGoogleClient" class="play">Save OAuth client on Oracle</button><a class="btn ghost" href="https://console.cloud.google.com/auth/clients" target="_blank" rel="noreferrer">Open Google Auth Platform</a></div></div><div id="googleConnectActions" class="authactions hidden"><button id="connectGoogle" class="play">Continue with Google</button><button id="editGoogleClient" class="ghost">Replace OAuth client</button></div><div id="googleGateError" class="notice err hidden"></div><div class="authnote">Sign in with <b>tapiwaguduza@gmail.com</b>. Oracle stores the offline refresh credential with restricted permissions and refreshes short-lived access tokens automatically. Set the Google OAuth app publishing status to <b>In production</b> before final authorization; Google test-mode refresh tokens expire after 7 days. Once production authorization is stored, this gate stays gone unless Google access is revoked, invalidated or the stored credential is deliberately removed.</div></div></div><main class="wrap">
<section class="topbar"><div><div class="eyebrow">Dial Health · Design Control Plane</div><h1>Screen Factory</h1><div class="subtitle">Evidence-backed screen generation with functional requirements delivered by Hermes, independent Dial Health design authority, deterministic rendering, live QA and platform-level packaging. Play latches an autonomous run that advances continuously across groups and platforms until paused, stopped, completed or held by a fail-closed gate.</div><div class="statusline"><span id="liveBadge" class="badge"><span class="dot"></span><span>Connecting…</span></span><span id="workerBadge" class="badge"><span class="dot"></span><span>Worker…</span></span><span id="modelBadge" class="badge"><span class="dot"></span><span>Models…</span></span><span id="autoBadge" class="badge"><span class="dot"></span><span>Autonomous run…</span></span><span class="badge"><span class="dot" style="background:#2563eb"></span><span>Hermes: functions only</span></span><span class="badge"><span class="dot" style="background:#8b5cf6"></span><span>Visual authority: independent</span></span></div></div><div class="controlbox"><button class="play" data-act="play" title="Latch continuous autonomous processing">▶ Play</button><button data-act="pause" title="Pause after the current atomic screen; keep the worker alive">Ⅱ Pause</button><button data-act="resume" title="Resume continuous autonomous processing">↻ Resume</button><button class="danger" data-act="stop" title="Cooperatively stop after the current atomic screen">■ Stop</button></div></section>
...
const $=id=>document.getElementById(id);let latestPing=null,latestSnap=null,refreshing=false,actionBusy=false,platformFilter='',platformExpanded=false,buExpanded=false,contractExpanded=false,googleClientEditing=false;
...
function gateError(message=''){const e=$('googleGateError');if(!message){e.classList.add('hidden');e.textContent='';return}e.textContent=message;e.classList.remove('hidden')}
function renderGoogleBootGate(oauth){const o=oauth||{};const gate=$('googleBootGate');if(o.authenticated){googleClientEditing=false;gate.classList.add('hidden');$('revokeGoogle').classList.remove('hidden');return}gate.classList.remove('hidden');$('revokeGoogle').classList.add('hidden');const setup=o.state==='SETUP_REQUIRED'||googleClientEditing;$('googleClientSetup').classList.toggle('hidden',!setup);$('googleConnectActions').classList.toggle('hidden',setup);$('googleGateCopy').innerHTML=setup?(googleClientEditing?'Replace the stored OAuth credential pair using the <b>client ID and current client secret from the same Google Web application</b>. This form will stay open while you type.':'One-time administrator setup is required before Google can issue a persistent offline credential. Create a <b>Web application</b> OAuth client in Google Auth Platform, register the callback below, then save the client here.'):'Authorize <b>'+esc(o.account_email||'tapiwaguduza@gmail.com')+'</b> once. After consent, the refresh credential stays on Oracle and the factory can archive while your phone is offline.'}
$('saveGoogleClient').addEventListener('click',async()=>{const id=$('googleClientId').value.trim(),secret=$('googleClientSecret').value.trim();if(!id||!secret)return gateError('Enter both the Google OAuth client ID and client secret.');const b=$('saveGoogleClient');b.disabled=true;gateError();try{await reqJson('/api/storage/google-auth/client',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({client_id:id,client_secret:secret})});googleClientEditing=false;$('googleClientSecret').value='';notice('Google OAuth client saved securely on Oracle');await refresh()}catch(e){gateError('OAuth client setup failed: '+e.message)}finally{b.disabled=false}});
$('connectGoogle').addEventListener('click',()=>{gateError();const target=new URL(api('/api/storage/google-auth/start?redirect=1&force=1'),location.origin).href;window.top.location.href=target});
$('editGoogleClient').addEventListener('click',()=>{googleClientEditing=true;renderGoogleBootGate(latestSnap?.google_oauth);gateError();setTimeout(()=>$('googleClientId')?.focus(),0);});
$('revokeGoogle').addEventListener('click',async()=>{if(!confirm('Revoke the stored Google Drive authorization? The boot authorization gate will return.'))return;try{await reqJson('/api/storage/google-auth/revoke',{method:'POST'});notice('Google Drive authorization revoked');await refresh()}catch(e){notice('Google revoke failed: '+e.message,'err')}});
async function completeGoogleCallback(){const q=new URLSearchParams(location.search),code=q.get('google_oauth_code'),state=q.get('google_oauth_state'),error=q.get('google_oauth_error');if(error){gateError('Google authorization failed: '+error);history.replaceState(null,'',location.pathname);return}if(!code||!state)return;try{gateError();$('googleGateCopy').textContent='Completing Google authorization on Oracle…';await reqJson('/api/storage/google-auth/exchange',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({code,state})});history.replaceState(null,'',location.pathname);notice('Google Drive connected for persistent offline archive access');await refresh()}catch(e){const msg=String(e.message||e);gateError('Google authorization could not be completed: '+msg);if(/client secret is invalid|invalid_client/i.test(msg)){googleClientEditing=true;renderGoogleBootGate(latestSnap?.google_oauth);$('googleGateCopy').innerHTML='Oracle reached Google, but Google rejected the stored OAuth credential pair. Re-enter the <b>client ID and current client secret from the same Web application</b>, save them, then authorize again. This form will stay open while you type.';}history.replaceState(null,'',location.pathname)}}
...
function kpis(s){const contractPct=pct(s.contract_ready,s.required_total).toFixed(1),u=s.generation_runtime?.usage||{},c=s.composition_pipeline||{};const perAccepted=Number(s.complete||0)>0?(Number(u.successful_requests||0)/Number(s.complete)).toFixed(2):'—';return [
 ...
 ['API requests',(u.successful_requests||0)+' / '+(u.request_attempts||0),perAccepted+' successful requests / accepted screen',''],
 ['Model tokens',Number(u.total_tokens||0).toLocaleString(),Number(u.reasoning_tokens||0).toLocaleString()+' reasoning tokens',''],
 ['Batch target',c.target_batch_size||'—',(c.min_batch_size||10)+'–'+(c.max_batch_size||40)+' adaptive family compositions',''],
...
function renderMission(s,cb){const next=s.next_task;const readiness=next?.contract_readiness;const stateClass=readiness?.ready?'good':'warn',run=s.execution_policy?.autonomous_run_latched,comp=s.composition_pipeline||{},eff=s.generation_efficiency||{};const batchMeta='Composer target '+(comp.target_batch_size||'—')+' screens/request · range '+(comp.min_batch_size||'—')+'–'+(comp.max_batch_size||'—');const usageMeta=(eff.request_attempts||0)+' API attempts · '+(eff.total_tokens||0).toLocaleString()+' tokens';return '<div class="mission-card"><div class="label">Next canonical screen</div><strong>'+(next?esc(next.screen_id+' — '+next.title):'Queue complete')+'</strong><div class="small">'+(next?esc(next.business_unit+' · '+next.platform):'No remaining REQUIRED tasks')+'</div></div><div class="mission-card"><div class="label">Contract gate</div><strong><span class="statepill '+stateClass+'">'+(readiness?.ready?'READY':'BLOCKED')+'</span></strong><div class="small">'+esc(readiness?.reason||'Durable functional contract available')+'</div></div><div class="mission-card"><div class="label">Composition batch</div><strong>'+esc(cb?.batch_id||'Prepared on demand')+'</strong><div class="small">'+esc(batchMeta)+'</div></div><div class="mission-card"><div class="label">Autonomous run</div><strong><span class="statepill '+(run?'good':'warn')+'">'+(run?'LATCHED':'OFF')+'</span></strong><div class="small">'+esc(usageMeta)+'</div></div>'}
...
function renderGallery(done,s){$('screenCount').textContent=s.complete+' complete';$('gallery').innerHTML=(done||[]).slice().reverse().map(x=>{const visual=x.asset_available?'<img loading="lazy" src="'+api('/api/screens/'+encodeURIComponent(x.task_id)+'/preview')+'">':'<div class="empty-visual"><div><b>Verified external evidence</b><br>'+esc(x.external_filename||'asset registered')+'</div></div>';return '<article class="thumb">'+visual+'<div class="meta"><b>'+esc(x.screen_id)+' — '+esc(x.title)+'</b><br>'+esc(x.business_unit)+' · '+esc(x.platform)+'<br><span class="statepill good" style="margin-top:5px">'+esc(x.status)+'</span> <span class="statepill" style="margin-top:5px">QA '+esc(x.basic_qa)+'</span>'+(x.implementation_ready?' <span class="statepill good" style="margin-top:5px">implementation-ready</span>':'')+(x.quality_score!==null?' <span class="statepill '+(Number(x.quality_score)>=90?'good':'warn')+'" style="margin-top:5px">quality '+esc(x.quality_score)+'/100</span>':'')+(x.generator_model?' <br><span style="color:#94a3b8;font-size:10px">'+esc(x.generator_model)+(x.quality_repaired?' · polished':'')+'</span>':'')+'</div></article>'}).join('')||'<div class="subtitle">No screens generated under the current design policy yet.</div>'}
async function refresh(){if(refreshing)return;refreshing=true;try{const snap=await reqJson('/api/dashboard');latestSnap=snap;const s=snap.status||{},cb=snap.current_batch||{},platforms=snap.platform_progress||[];const cls=statusClass(s.state);$('liveBadge').className='badge '+cls;$('liveBadge').innerHTML='<span class="dot"></span><span>Factory '+esc(s.state)+'</span>';$('workerBadge').className='badge '+(snap.worker_service==='active'?'live':'stop');$('workerBadge').innerHTML='<span class="dot"></span><span>Worker '+esc(snap.worker_service||'unknown')+'</span>';const gen=s.generation_runtime||{},models=gen.active_models||[],quota=gen.usage?.quota_state;$('modelBadge').className='badge '+(quota?'warn':(gen.enabled&&models.length?'live':'warn'));$('modelBadge').innerHTML='<span class="dot"></span><span>'+(quota?'Quota blocked':('Models '+esc(models.length?models.length+' active':'not ready')))+'</span>';$('modelBadge').title=(quota?'OpenRouter account quota: '+quota+' until '+(gen.usage?.quota_retry_after||'reset unknown')+' · ':'')+models.join(' · ');const auto=s.execution_policy?.autonomous_run_latched;$('autoBadge').className='badge '+(auto?'live':(s.requested_state==='PAUSED'?'warn':'stop'));$('autoBadge').innerHTML='<span class="dot"></span><span>Autonomous '+(auto?'latched':(s.requested_state==='PAUSED'?'paused':'off'))+'</span>';$('lastUpdated').textContent='Updated '+fmtTime(snap.server_time);$('kpis').innerHTML=kpis(s);$('overallText').textContent=s.complete+' / '+s.required_total+' · '+s.percent+'%';$('overallFill').style.width=pct(s.complete,s.required_total)+'%';$('contractText').textContent=s.contract_ready+' / '+s.required_total+' · '+pct(s.contract_ready,s.required_total).toFixed(1)+'%';$('contractFill').style.width=pct(s.contract_ready,s.required_total)+'%';$('mission').innerHTML=renderMission(s,cb);renderStorage(s);renderGoogleBootGate(snap.google_oauth);syncControls(s);renderPing(snap.current_ping);renderProgressLists(s,platforms);renderPlatforms(platforms,snap.platform_packages||[]);renderPackages(snap.platform_packages||[]);renderEvents(snap.recent_events||[]);renderGallery(snap.recent_tasks||[],s)}catch(e){$('liveBadge').className='badge fail';$('liveBadge').innerHTML='<span class="dot"></span><span>Connection error</span>';notice('Live refresh failed: '+e.message,'err')}finally{refreshing=false}}
completeGoogleCallback().finally(()=>refresh());setInterval(refresh,2500);
</script></body></html>`;
ensureScreenFactory();
