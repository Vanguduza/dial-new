...
function loadState(root) {
  return readJson(STATE_REL, { schema_version: 1, connected_notice_fingerprint: null, progress_cursor: null, processed: [], last_mission_state: null, last_blocker_hash: null, chat_id: null }, root);
}
function saveState(state, root) {
  writeJsonAtomic(STATE_REL, { ...state, processed: (state.processed || []).slice(-MAX_PROCESSED), updated_at: now() }, root);
}
...
export async function hermesWhatsAppOperatorTick({ root, fetchImpl = fetch, state = loadState(root) } = {}) {
  ...
  for (const msg of Array.isArray(messages) ? messages : []) {
    ...
    if (!messageId || !body || seen(state, messageId)) continue;
    ...
    try {
      const routed = await executeOperatorTextCommand(body, { root, channel: 'whatsapp', actor: `self:${id.fingerprint}`, requestSeed: messageId, cursor: state.progress_cursor });
      ...
      event(root, 'HERMES_WHATSAPP_OPERATOR_COMMAND_COMPLETED', { command: routed.command?.kind || 'unknown', message_id_hash: hash(messageId).slice(0, 24), identity_fingerprint: id.fingerprint });
    } catch (err) {
  ...
  if (chatId && state.connected_notice_fingerprint !== id.fingerprint) {
    await bridgeSend(chatId, 'DIAL operator control is connected. Send HELP for commands.', fetchImpl);
  ...
  if (chatId && mission) {
    ...
    const important = ['BLOCKED_OWNER', 'WAITING_RUNTIME', 'COMPLETE'].includes(mission.state);
