...
function researchProjection(root,unitMap){
 const record=readJson(`knowledge/research/union-alpha/by-unit/${unitMap.unit_lineage_id}.json`,null,root);
 if(!record||record.status!=='RESEARCH_READY'||record.unit_revision_hash!==unitMap.unit_revision_hash)return{state:'NOT_READY',authority:'NON_AUTHORITATIVE_ENGINEERING_GUIDANCE'};
 const shared=(record.shared_artifact_refs||[]).map((id)=>readJson(`knowledge/research/union-alpha/shared/${id}.json`,null,root)).filter((x)=>x?.admission?.state==='ADMITTED'&&x.applicability?.unit_lineage_ids?.includes(unitMap.unit_lineage_id));
 if(!shared.length)return{state:'REFUSED',reason:'ADMITTED_SHARED_RESEARCH_MISSING'};
 const research=shared[0].claims; const roles={}; for(const [role,row] of Object.entries(research?.roles||{})){roles[role]={findings:bounded(row.findings,3).map((x)=>({claim:boundedText(x?.claim,1800),source_refs:bounded(x?.source_refs,8),confidence:x?.confidence??null,freshness_note:boundedText(x?.freshness_note,500)})),risks:bounded(row.risks,3).map((x)=>boundedText(x,1000)),recommendations:bounded(row.recommendations,3).map((x)=>boundedText(x,1000)),unknowns:bounded(row.unknowns,3).map((x)=>boundedText(x,1000))};}
 return{state:'RESEARCH_READY',authority:'NON_AUTHORITATIVE_ENGINEERING_GUIDANCE',mission_id:record.mission_id,model_id:record.model_id,research_hash:record.research_hash,coverage_hash:record.coverage_hash,artifact_evidence_hash:record.artifact_evidence_hash,shared_artifact_refs:record.shared_artifact_refs,roles,cross_role_synthesis:research?.cross_role_synthesis||null,freshness:shared[0].freshness,invalidation:shared[0].invalidation,polarities:shared[0].polarities};
}
export function buildContextCapsules({root=DEFAULT_CONTROL_HOME,packetId,unitMap,graphResolution,skillPlan}={}){
 ...
 const defs={
  ...
  IMPLEMENTATION:{api_contracts:bounded(unitMap.implementation_map.api_contracts),recommended_sequence:bounded(unitMap.implementation_map.recommended_sequence),selected_resource_descriptors:resources,selected_skill_descriptors:skills,structural_reality:unitMap.structural_reality_map||null},
  RESEARCH:researchProjection(root,unitMap),
