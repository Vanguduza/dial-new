...
export const ALLOWED_OPERATION_JOBS = Object.freeze(['service_health', 'repo_integrity', 'evidence_prepare', 'backup_verify']);
...
function run(command, args, cwd) {
  try { return { ok: true, stdout: execFileSync(command, args, { cwd, encoding: 'utf8', stdio: ['ignore', 'pipe', 'pipe'], timeout: 120000, env: { ...process.env, GIT_OPTIONAL_LOCKS: '0' } }).trim() }; }
  catch (error) { return { ok: false, stdout: String(error?.stdout || '').trim(), stderr: String(error?.stderr || error?.message || error).trim().slice(0, 4000), status: error?.status ?? null }; }
}
...
function repoIntegrity(project) {
  const head = run('git', ['rev-parse', 'HEAD'], project.repo_dir);
  const branch = run('git', ['branch', '--show-current'], project.repo_dir);
  const status = run('git', ['status', '--porcelain=v1'], project.repo_dir);
  const diffCheck = run('git', ['diff', '--check'], project.repo_dir);
  const inside = run('git', ['rev-parse', '--is-inside-work-tree'], project.repo_dir);
  return {
    check: 'repo_integrity',
    ok: head.ok && branch.ok && status.ok && diffCheck.ok && inside.stdout === 'true',
    head: head.stdout || null,
    branch: branch.stdout || null,
    dirty: Boolean(status.stdout),
    dirty_paths: status.stdout ? status.stdout.split('\n').filter(Boolean).slice(0, 200) : [],
    diff_check_green: diffCheck.ok,
    repository_detected: inside.stdout === 'true',
    errors: [head, branch, status, diffCheck, inside].filter((item) => !item.ok).map((item) => item.stderr).filter(Boolean),
  };
}

function serviceHealth(project) {
  const services = Array.isArray(project.services) ? project.services : [];
  const results = services.map((service) => {
    const active = run('systemctl', ['--user', 'is-active', service], project.repo_dir);
    const enabled = run('systemctl', ['--user', 'is-enabled', service], project.repo_dir);
...
function backupVerify(root) {
  ...
  return {
    check: 'backup_verify',
    ok: checkpointExists && backups.length > 0 && backups[0].size_bytes > 0 && backups[0].mode === '600',
    checkpoint_pointer_present: Boolean(pointer),
    active_checkpoint_present: checkpointExists,
    hermes_backups: backups.slice(0, 10),
    latest_backup_present: backups.length > 0,
  };
}

function evidencePrepare(project, root) {
  return {
    check: 'evidence_prepare',
    ok: true,
    ...
    repository: repoIntegrity(project),
    services: serviceHealth(project),
...
export async function runOperationsJob({ job, projectSlug = 'dial', root, useApi = false } = {}) {
  ...
  let evidence;
  if (job === 'service_health') evidence = serviceHealth(project);
  if (job === 'repo_integrity') evidence = repoIntegrity(project);
  ...
  if (job === 'evidence_prepare') evidence = evidencePrepare(project, root);
  ...
  const record = { schema_version: 1, authority: OPERATIONS_AUTHORITY, development_authority: false, project: project.slug, job, evidence, api_summary, observed_at: observedAt };
  writeJsonAtomic(runRel(project.slug, job, observedAt), record, root);
  writeJsonAtomic(latestRel(project.slug, job), record, root);
...
function defaultSchedules() {
  ...
  return {
    schema_version: 1,
    policy: 'DETERMINISTIC_FIRST_API_OPTIONAL_NON_AUTHORITATIVE',
    schedules: [
...
export function ensureOperationsSchedules(root) {
  ...
  if (existing?.schema_version === 1 && Array.isArray(existing.schedules)) return existing;
  const schedules = defaultSchedules(); writeJsonAtomic(SCHEDULES_REL, schedules, root); return schedules;
}
...
export async function runDueOperations({ root } = {}) {
  ...
  if (completed.length) writeJsonAtomic(SCHEDULES_REL, { ...schedules, updated_at: now() }, root);
  return completed;
}
...
export function operationsStatus(root) {
  ...
  return { schema_version: 1, authority: OPERATIONS_AUTHORITY, development_authority: false, api: operationsApiStatus(root), projects, schedules, last_runs: Object.fromEntries(projects.map((p) => [p.slug, Object.fromEntries(ALLOWED_OPERATION_JOBS.map((job) => [job, readJson(latestRel(p.slug, job), null, root)]))])), observed_at: now() };
}
