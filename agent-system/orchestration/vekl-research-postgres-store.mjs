...
const SQL = Object.freeze({
  ...
  lease: `WITH candidate AS (SELECT p.packet_id FROM vekl_research_packets p JOIN vekl_research_missions m ON m.mission_id=p.mission_id WHERE m.state='READY' AND (p.state IN ('READY','RETRY') OR (p.state='LEASED' AND p.lease_expires_at <= to_timestamp($1/1000.0))) AND (p.lease_expires_at IS NULL OR p.lease_expires_at <= to_timestamp($1/1000.0)) ORDER BY p.ordinal FOR UPDATE OF p SKIP LOCKED LIMIT 1) UPDATE vekl_research_packets p SET state='LEASED',worker_id=$2,lease_id=$3,lease_expires_at=to_timestamp($4/1000.0) FROM candidate WHERE p.packet_id=candidate.packet_id RETURNING p.*`,
