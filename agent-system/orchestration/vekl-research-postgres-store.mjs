...
const SQL = Object.freeze({
  ...
  completionBundle: `SELECT p.packet_id,p.packet_hash,p.packet_json,p.resume,p.state,p.worker_id,p.lease_id,p.lease_expires_at,
    COALESCE((SELECT jsonb_agg(jsonb_build_object(
      $$lease_id$$,e.lease_id,$$evidence_kind$$,e.evidence_kind,$$claims$$,e.claims,$$sources$$,e.sources,$$evidence_hash$$,e.evidence_hash
    ) ORDER BY e.created_at) FROM vekl_research_evidence e WHERE e.packet_hash=p.packet_hash),$$[]$$::jsonb) AS evidence,
    COALESCE((SELECT jsonb_agg(jsonb_build_object(
      $$lease_id$$,ev.lease_id,$$event_kind$$,ev.event_kind,$$payload$$,ev.payload,$$created_at$$,ev.created_at
    ) ORDER BY ev.event_id)
      FROM vekl_research_events ev
      WHERE ev.lease_id=p.lease_id
         OR ev.lease_id IN (
           SELECT DISTINCT e2.lease_id
           FROM vekl_research_evidence e2
           WHERE e2.packet_hash=p.packet_hash AND e2.lease_id IS NOT NULL
         )
    ),$$[]$$::jsonb) AS events
