...
function fingerprint(repoDir,root){const h=crypto.createHash('sha256');for(const rel of TRUTH_FILES){const p=path.join(repoDir,rel);h.update(rel);h.update('\0');if(fs.existsSync(p))h.update(fs.readFileSync(p));h.update('\0');}for(const rel of ['missions/dial-development-root.json','state/active-checkpoint.json']){const value=readJson(rel,null,root);h.update(rel);h.update('\0');if(value)h.update(JSON.stringify(value));h.update('\0');}return h.digest('hex');}
...
function validForecast(value,sourceIds){if(!value||value.schema_version!==1||!Array.isArray(value.items))return false;const allowed=new Set(sourceIds);for(const item of value.items.slice(0,8)){item.preferred_source_ids=(item.preferred_source_ids||[]).filter((x)=>allowed.has(x));item.task_classes=Array.isArray(item.task_classes)?item.task_classes:[];item.technologies=Array.isArray(item.technologies)?item.technologies:[];item.research_questions=Array.isArray(item.research_questions)?item.research_questions:[];item.search_queries=Array.isArray(item.search_queries)?item.search_queries:[];item.risks=Array.isArray(item.risks)?item.risks:[];}value.items=value.items.slice(0,8);return true;}
...
export async function refreshAheadOfWorkResearch({repoDir=DEFAULT_REPO,root=DEFAULT_CONTROL_HOME,force=false,planner=runProjectAwareResearchForecast,fetchImpl=fetch,maxResources=18}={}){
  ...
  if(!force&&current?.plan_fingerprint===fp&&current?.created_at&&Date.now()-Date.parse(current.created_at)<maxAge)return {...current,refresh_state:'CURRENT'};
