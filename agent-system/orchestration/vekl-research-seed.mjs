...
export function inferResearchContexts(subject, unit = {}, binding = {}) {
  ...
  return {
    guided_frontend_context: {
      applicable: frontend,
      design_provenance_mode: ['NEW_DIAL_DESIGN', 'REFERENCE_INSPIRED_DIAL_NATIVE', 'LOCKED_BASELINE_REPAIR'],
      design_iteration_phase: ['EXPLORE', 'CONVERGE', 'RECONSTRUCT'],
      evidence_required: ['SCREEN_QUALITY_PACKET', 'STRUCTURED_CRITICS', 'FREEDOM_BUDGET', 'EXTERNAL_REFERENCE_NON_AUTHORITY', 'VISUAL_AUTHORITY', 'INTERACTION_DESIGN_PREFLIGHT', 'INTERACTION_MOTION_INTELLIGENCE'],
      critic_taxonomy: ['HIERARCHY','COMPOSITION','ACCESSIBILITY','RESPONSIVENESS','BRAND_COHERENCE','INTERACTION_CLARITY','MOTION_PURPOSE','GESTURE_CONFLICT','PLATFORM_BEHAVIOR','STATE_BEHAVIOR','OPTICAL_POLISH','PERFORMANCE','IMPLEMENTATION_PARITY'],
      freedom_budget_dimensions: ['LAYOUT_COMPOSITION','VISUAL_TREATMENT','MICRO_INTERACTIONS','INTERACTION_COMPOSITION','MOTION_EXPRESSION','ILLUSTRATION','TYPOGRAPHIC_EXPRESSION'],
      external_reference_criteria: ['FEATURE_RELEVANCE','UX_RELEVANCE','LICENSE_CONTEXT','PROVENANCE','SECURITY','ABSTRACT_LESSON_VALUE','ANTI_PATTERN_VALUE'],
      ...
      reconstruction_requirements: ['VISUAL_TO_FUNCTIONAL_MAPPING','INTERACTION_BINDING','RESPONSIVE_PARITY','NO_DEAD_CONTROLS','STATE_BEHAVIOR_PARITY','REDUCED_MOTION','PLATFORM_NATIVE_BEHAVIOR'],
      stitch_provider_evidence: ['PROVIDER_PROVENANCE','SCREEN_OUTPUT_HASH','ORCHESTRATED_EXECUTION','OUTAGE_NO_SILENT_SUBSTITUTION','INTERACTION_CONTRACT_HASH'],
      interaction_motion_research_profile: {
        status: 'FIRST_CLASS_FRONTEND_RESEARCH',
        required_topics: ['modern_interaction_patterns','motion_hierarchy','microinteractions','responsive_recomposition','platform_native_behavior','gesture_conflicts','state_feedback','offline_degraded_recovery','accessibility','reduced_motion','performance_budgets','professional_saas_ergonomics','optical_polish'],
        open_world_discovery: true,
        discovery_mode: 'DISCOVERED_TO_QUALIFIED_TO_ADMITTED',
        pattern_registry: 'agent-system/registries/INTERACTION_MOTION_PATTERN_REGISTRY.json',
        source_registry: 'agent-system/registries/INTERACTION_MOTION_SOURCE_REGISTRY.json',
        external_reference_policy: 'REFERENCE_AND_INSPIRATION_ONLY',
      },
