...
export function canonicalActionCatalog(task) {
  ...
  const routeDerived=routes.filter((route)=>!String(route).startsWith('handoff://')).map((route,index)=>({label:routeLabel(route),source:'documented_route',source_index:index,route}));
...
export function compositionRecipeFor(task) {
  ...
  if(a.includes('command centre')||a.includes('command center')||a.includes('professional command'))return ['metrics','list','highlight','action_grid'];
  if(a.includes('professional summary')||a.includes('record workspace'))return ['detail','metrics','list','highlight'];
  if(a.includes('professional workbench')||a.includes('workspace')||a.includes('composer'))return ['detail','form','highlight','action_grid'];
  if(a.includes('professional queue')||a.includes('work queue')||a.includes('inbox')||a.includes('tracker'))return ['segmented','list','highlight','action_grid'];
  if(a.includes('professional schedule')||a.includes('resource board'))return ['segmented','timeline','list','action_grid'];
  if(a.includes('professional search'))return ['search','list','detail','action_grid'];
  if(a.includes('reconciliation'))return ['metrics','list','detail','action_grid'];
  if(a.includes('timeline/journey'))return ['timeline','metrics','list','highlight'];
  if(a.includes('professional detail'))return ['detail','metrics','list','action_grid'];
  return String(task.business_unit||'')==='My Health' ? ['hero','highlight','list'] : ['detail','list','highlight','action_grid'];
}
...
.dh-professional-screen{background:#f4f7fa;padding:0 24px 24px}.dh-professional-screen .dh-shell{max-width:1280px}.dh-professional-header{min-height:72px;margin:0 -24px 18px;padding:0 24px;background:#fff;border-bottom:1px solid var(--dh-border);box-shadow:0 5px 18px rgba(8,44,92,.035);display:grid;grid-template-columns:auto minmax(0,1fr) auto;gap:18px}.dh-professional-title{min-width:0}.dh-professional-title>span{display:block;font-size:10px;font-weight:850;letter-spacing:.09em;text-transform:uppercase;color:var(--dh-teal-deep)}.dh-professional-title>strong{display:block;margin-top:2px;font-size:20px;line-height:1.15;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.dh-role-context{font-size:11px;font-weight:750;color:var(--dh-muted);background:#eef4f7;border:1px solid var(--dh-border);border-radius:999px;padding:7px 10px;white-space:nowrap}.dh-professional-screen .dh-section{margin-top:0}.dh-professional-screen .dh-list-row{min-height:60px!important;padding:9px 11px!important}.dh-professional-screen .dh-list-title{font-size:14px}.dh-professional-screen .dh-list-meta{font-size:11px}.dh-professional-screen .dh-dsl-action-tile{min-height:72px!important}.dh-professional-screen .dh-inline-cta{min-height:42px}.dh-professional-screen .dh-more{background:#fff;border:1px solid var(--dh-border);border-radius:14px;padding:4px 12px;margin-top:12px}
@media(min-width:900px){.dh-professional-screen .dh-dsl-body{display:grid;grid-template-columns:repeat(12,minmax(0,1fr));gap:14px;align-items:start}.dh-professional-screen .dh-dsl-body>section{grid-column:span 6}.dh-professional-screen .dh-dsl-body>section:first-child,.dh-professional-screen .dh-dsl-body>.dh-dsl-actions,.dh-professional-screen .dh-dsl-body>.dh-search-section{grid-column:1/-1}.dh-professional-screen .dh-action-grid{grid-template-columns:repeat(4,minmax(0,1fr));gap:10px}.dh-professional-screen .dh-metric-grid{grid-template-columns:repeat(4,minmax(0,1fr))}.dh-professional-screen .dh-section-head{margin-bottom:8px}.dh-professional-screen .dh-section-title{font-size:16px}.dh-professional-screen .dh-card{box-shadow:0 4px 14px rgba(8,44,92,.04)}}
@media(max-width:899px){.dh-professional-screen{padding:0 16px 18px}.dh-professional-header{margin:0 -16px 14px;padding:0 16px;grid-template-columns:auto minmax(0,1fr)}.dh-role-context{display:none}.dh-professional-title>strong{font-size:17px}.dh-professional-screen .dh-action-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}
...
export function compileCompositionToPacket(task, rawComposition) {
  ...
  const professional=String(task.business_unit||'')!=='My Health';
  const roleContext=contractList(task.roles).slice(0,2).join(' · ')||'Authorised workspace';
  const consumerHeader=homeNav?`<header class="dh-dsl-header dh-home-header" data-ui="app-header"><div class="dh-brand-lockup">${brandMark}<span>Dial Health</span></div>${headerStatus}</header>`:`<header class="dh-dsl-header dh-subpage-header" data-ui="app-header"><span class="dh-back-glyph" aria-hidden="true">‹</span><strong>${esc(task.title)}</strong><span class="dh-header-spacer" aria-hidden="true"></span></header>`;
  const professionalHeader=`<header class="dh-dsl-header dh-professional-header" data-ui="app-header"><div class="dh-brand-lockup">${brandMark}<span>Dial Health</span></div><div class="dh-professional-title"><span>${esc(task.business_unit||'Professional workspace')}</span><strong>${esc(task.title)}</strong></div><span class="dh-role-context">${esc(roleContext)}</span></header>`;
  const header=professional?professionalHeader:consumerHeader;
  const rootClass=professional?'dh-professional-screen':(homeNav?'dh-home-screen dh-has-bottom-nav':'dh-subpage-screen');
  const semantic_html=`<main class="dh-screen ${rootClass}" data-ui="screen" data-screen-id="${esc(task.screen_id)}" data-business-unit="${esc(task.business_unit)}" data-platform="${esc(task.platform)}"><div class="dh-shell">${header}<div class="dh-dsl-body">${regions}</div>${moreInfo}${moreActions}${bottomNav}</div></main>`;
  ...
  const packet={
    ...
    experience_profile:{information_density:professional?'MODERATE_TO_HIGH':'LOW_TO_MODERATE',progressive_disclosure:professional?false:true,composition_dsl:SCREEN_COMPOSITION_DSL_VERSION,audience:professional?'PROFESSIONAL_OPERATOR':'CONSUMER_PATIENT_FAMILY'},
