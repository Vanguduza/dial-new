...
Authoritative screen compilation is exact-model and fail-closed: GPT-5.6 Sol first, then exact Claude Sonnet 5. The model produces an evidence-backed implementation packet; Playwright deterministically renders the standalone image from semantic HTML/CSS. Hermes remains the control-plane owner for heartbeat, canonical queue ordering, retries, deterministic QA, state persistence and packaging. OpenRouter remains auxiliary-only and cannot generate or approve product truth.
