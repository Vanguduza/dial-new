...
import { activateCompiledGraph, loadActiveGraph } from './canon-graph-compiler.mjs';
import { findUnit, hashObject, loadRegistry, registryHash } from './knowledge-graph-core.mjs';
import { classifyEngineeringResourcesTask, evaluateEngineeringResourceHardEligibility } from './engineering-resource-resolver.mjs';
...
const N8N_PATTERN_REL='agent-system/engineering-knowledge/automation/N8N_WORKFLOW_PATTERN_REGISTRY.json';
const N8N_CAP_REL='agent-system/engineering-knowledge/automation/n8n-node-capability-map.json';
const N8N_SEC_REL='agent-system/engineering-knowledge/automation/n8n-security-rules.json';

function tokens(value){return [...new Set(String(value||'').toLowerCase().match(/[a-z0-9][a-z0-9_.-]{1,}/g)||[])].sort();}
function hashVector(text,dim=256){ const out=new Float64Array(dim); for(const t of tokens(text)){ let h=2166136261; for(let i=0;i<t.length;i++){h^=t.charCodeAt(i);h=Math.imul(h,16777619)>>>0;} const idx=h%dim; const sign=((h>>>8)&1)?1:-1; out[idx]+=sign; } let n=0;for(const x of out)n+=x*x;n=Math.sqrt(n)||1;return [...out].map((x)=>x/n); }
...
function buildEnvelope(repoDir,graph,policy,routeRegistry){
 ...
 p.concern_classifier.prompt_or_schema_hash=hashObject({rules:'deterministic concern routes v1',routes:routeHash}); p.retrieval_routes={registry_version:routeRegistry.registry_version,registry_hash:routeHash};
