...
export function runVeklArchitectureCheck({repoDir=process.cwd()}={}){
 ...
 try{
  ...
  results.push(row(26,'bounded provenance-bearing worker context',capsuleKeys.length===8&&capsuleKeys.includes('research_hash')&&manifest.resources.length<=24&&trace.worker_delivery_hash===delivery.worker_delivery_hash&&trace.activation.manifest_hash===manifest.manifest_sha256,{capsules:capsuleKeys,worker_delivery_hash:delivery.worker_delivery_hash}));
