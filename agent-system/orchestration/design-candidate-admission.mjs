...
export function admitDesignCandidate({candidate,authorityConforms,requiredStatesPresent,donorSemanticsPreserved=true,designNormalizationEvidence=null,changeBudgetSatisfied=null}={}){if(!candidate)throw new Error('candidate required');const failures=[];if(authorityConforms!==true)failures.push('DESIGN_AUTHORITY_DRIFT');if(requiredStatesPresent!==true)failures.push('REQUIRED_STATES_MISSING');if(donorSemanticsPreserved!==true)failures.push('DONOR_SEMANTICS_NOT_PRESERVED');if(candidate.frontend_design_execution_packet_hash&&designNormalizationEvidence?.status!=='NORMALIZED')failures.push('DESIGN_NORMALIZATION_REQUIRED');if(candidate.change_budget_hash&&changeBudgetSatisfied!==true)failures.push('CHANGE_BUDGET_NOT_SATISFIED');if(failures.length)return{ok:false,failures};const base={schema_version:1,candidate_id:candidate.candidate_id,candidate_hash:candidate.candidate_hash,unit_lineage_id:candidate.unit_lineage_id,unit_revision_hash:candidate.unit_revision_hash,design_authority_projection_hash:candidate.design_authority_projection_hash,frontend_design_execution_packet_hash:candidate.frontend_design_execution_packet_hash||null,design_normalization_evidence_hash:designNormalizationEvidence?.content_hash||null,accepted_screen_refs:candidate.screen_refs,accepted_at:new Date().toISOString()};return{ok:true,manifest:{...base,manifest_hash:hashObject({...base,accepted_at:null})}};}
