...
export function compileGraphContent(repoDir=DEFAULT_REPO){
 ...
 for(const f of features){
   ...
   const c=contractByFeature.get(f.feature_id); if(c){
     ...
     for(const referenceId of uniq(c.donor_refs||f.donor_refs||[])){ const row=donorRows.get(referenceId)||null; const isExternal=Boolean(row?.classification==='EXTERNAL_REFERENCE'||/^DONOR-|^REF-/.test(referenceId)); if(!isExternal) continue; const dref=add('EXTERNAL_REFERENCE',referenceId,{reference_id:referenceId,registered:row!==null,authority:'NON_AUTHORITATIVE_REFERENCE_ONLY'},{object_type:'EXTERNAL_REFERENCE',object_id:referenceId,stable_ref:DONOR_REL,expected_content_hash:row?hashObject(row):hashObject({unregistered_external_reference:referenceId})}); edge(dref,'informs_non_authoritatively',fref,{stable_ref:CONTRACT_REL,source_content_hash:hashObject(c)}); }
