...
import { runAuthWorkflow } from './auth/workflow.mjs';
import { PROFILES, DEFAULT_PROFILE } from './lib/readiness.mjs';

const MODES = ['dry-run', 'verify', 'apply', 'repair', 'rollback', 'self-test', 'determinism', 'fault-injection', 'plan', 'auth'];

export function parseArgs(argv) {
  const out = { mode: null, role: null, profile: DEFAULT_PROFILE, resume: null, json: false, fast: false, out: null, controlHome: null, repo: null, runId: null, rollbackId: null, keep: false };
  for (let i = 0; i < argv.length; i += 1) {
    const a = argv[i];
    const next = () => argv[++i];
    if (a === '--role') out.role = next();
    else if (a === '--profile') out.profile = next();
    else if (a === '--resume') out.resume = next();
  ...
  if (!PROFILES[out.profile]) throw new Error(`unknown readiness profile ${out.profile}`);
  if (out.resume && out.mode !== 'auth') throw new Error('--resume is valid only with --auth');
  return out;
}
...
export async function main(argv = process.argv.slice(2)) {
  ...
  if (args.mode === 'auth') {
    const r = runAuthWorkflow({ manifest, role, repoDir, controlHome, resumeToken: args.resume, profileClasses: PROFILES[args.profile] });
    emit(r); writeOut(r);
    if (!args.json) {
      for (const item of r.human_login_required) console.log(`OWNER ACTION ${item.credential_id}: ${item.instruction}`);
      console.log(`AUTH: ${r.certified.length} certified, ${r.failed.length} failed, ${r.human_login_required.length} require owner login`);
      if (r.resume_command) console.log(`Resume after login: ${r.resume_command}`);
    }
    return r.failed.length ? 1 : r.human_login_required.length ? 2 : 0;
  }

  if (args.mode === 'dry-run' || args.mode === 'plan' || args.mode === 'apply' || args.mode === 'repair') {
    let checks = [];
    if (args.mode === 'repair') { const pre = await certify({ repoDir, controlHome, manifest, role, runId, log, fast: true, skipRepositoryGates: true, profile: args.profile }); checks = pre.checks; }
    ...
    if (args.mode === 'apply' || args.mode === 'repair') { const post = await certify({ repoDir, controlHome, manifest, role, runId, log, fast: true, skipRepositoryGates: true, profile: args.profile }); if (!args.json) console.log(`\npost-apply ${args.profile} verdict: ${post.overall_status} — ${post.verdict_reason}`); return post.overall_status === 'GREEN' ? 0 : 1; }
    return 0;
  }
  if (args.mode === 'verify') {
    const report = await certify({ repoDir, controlHome, manifest, role, runId, log, fast: args.fast, profile: args.profile });
