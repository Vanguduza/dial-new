    ...
    switch (job.kind) {
      ...
      case 'STRUCTURAL_SNAPSHOT': {
        ...
        receipt.result = {
          ...
          repo_sha: repoSha,
          nodes: snapshot.nodes?.length ?? null,
          edges: snapshot.edges?.length ?? null,
          snapshot_hash: published?.snapshot_hash || snapshot.snapshot_hash || snapshot.identity_hash || null,
          normalized_graph_hash: published?.normalized_graph_hash || snapshot.normalized_graph_hash || null,
          provider_graph_hash: published?.provider_graph_hash || snapshot.provider_graph_hash || null,
          source_file_manifest_hash: published?.source_file_manifest_hash || snapshot.source_file_manifest_hash || null,
          published: Boolean(published?.snapshot_hash),
          authority: 'SUBORDINATE_STRUCTURAL_EVIDENCE'
        };
        if (!receipt.result.snapshot_hash || !receipt.result.normalized_graph_hash) {
          throw Object.assign(new Error('structural snapshot publish did not return immutable hash evidence'), { code: 'STRUCTURAL_HASH_MISSING' });
        }
        break;
      }
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  else if (argv.includes('--kind')) { const i = argv.indexOf('--kind'); job = { kind: argv[i + 1] }; }
