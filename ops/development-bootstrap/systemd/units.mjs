...
export function evaluateUnitHardening({ show, repoDir = null, repoRwExpected = false, unit = '' } = {}) {
  ...
  const envFiles = parseEnvironmentFiles(prop(show, 'EnvironmentFiles'));
  const failures = [];
  ...
  return { unit, no_new_privileges: nnp, protect_system: protectSystem || null, protect_home: protectHome || null, private_tmp: privateTmp, unset_environment_ok: unsetOk, api_key_in_environment: keyLeak, repo_read_write: repoRw, repo_rw_expected: repoRwExpected, environment_files: envFiles, failures, ok: failures.length === 0, severity: keyLeak ? 'P0' : (repoRw && !repoRwExpected ? 'P1' : null) };
}
...
export function parseEnvironmentFiles(value) {
  ...
  return matches.map((m) => m[1].replace(/^-/, '').replace(/^['"]|['"]$/g, '')).filter(Boolean);
}
...
export function environmentFileModes(files) {
  ...
  for (const f of files) {
    ...
    if (!st.ok) { out.push({ file: f, exists: false, mode: null, ok: false }); continue; }
    const [mode, owner] = st.output.split(/\s+/);
    out.push({ file: f, exists: true, mode, owner, ok: mode === '600' || mode === '400' });
  }
  return out;
}
  ...
  for (const s of services) {
    ...
    if (healthy && !s.unit.endsWith('.timer')) {
      ...
      if (h.environment_files.length) {
        const modes = environmentFileModes(h.environment_files);
        checks.push(check({ id: `systemd.${s.unit}.environment-file-modes`, domain, title: `${s.unit} EnvironmentFile(s) exist with mode 600/400 (values never read)`, status: modes.every((m) => m.ok) ? STATUS.PASS : STATUS.FAIL, criticality: CRITICALITY.MANDATORY, readiness_class: s.readiness_class, evidence: { files: modes }, remediation: 'chmod 600 the credential environment file; reinstall from the canonical installer', severity: modes.some((m) => m.exists && !m.ok) ? 'P1' : null }));
      }
    }
  }
