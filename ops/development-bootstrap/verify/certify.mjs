...
export async function certify({ repoDir, controlHome, manifest, role, runId, log, fast = false, skipRepositoryGates = false, gapRegister = [], profile = DEFAULT_PROFILE }) {
  ...
  if (resolved.role !== 'UNKNOWN' && resolved.role !== 'provider-container') {
    add([check({ id: 'role.role-file', domain: 'Role guard', title: 'host role declared by the canonical key=value role file (not hostname or env alone)', status: resolved.source === 'role_file' && resolved.role_file_format === 'CANONICAL' ? STATUS.PASS : STATUS.FAIL, criticality: CRITICALITY.REQUIRED, readiness_class: 'CORE_DEVELOPMENT_REQUIRED', evidence: { source: resolved.source, format: resolved.role_file_format || null, fabric: resolved.role_file_fabric || null, revision: resolved.role_file_revision || null }, remediation: 'bash deploy/oracle/execution-fabric/install-host-role.sh <class>' })]);
  }
  const mismatch = [];
  ...
  const immutableProbes = { 'oracle-admin': ['REPOSITORY_WRITE', 'HERMES_RUNTIME', 'OWNER_CONTROL', 'DETERMINISTIC_VERIFY'], 'vekl-worker': ['OWNER_CONTROL', 'HERMES_RUNTIME', 'EXTERNAL_QUEUE', 'HEAVY_BUILD', 'REPOSITORY_WRITE'], 'dial-hermes-control': ['OCI_RECOVERY', 'BREAK_GLASS', 'SSH_BROKER'] };
  const leaks = [];
  ...
  add([check({ id: 'role.immutable-roles-fail-closed', domain: 'Role guard', title: 'all three immutable roles refuse out-of-role work (project work on oracle-admin, control/heavy work on vekl-worker, recovery authority on dial-hermes-control)', status: leaks.length ? STATUS.FAIL : STATUS.PASS, criticality: CRITICALITY.MANDATORY, readiness_class: 'CORE_DEVELOPMENT_REQUIRED', evidence: { probes: immutableProbes, leaks } })]);
