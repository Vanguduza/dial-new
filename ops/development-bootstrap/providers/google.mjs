...
export function certifyGoogle({ role, repoDir }) {
  ...
  for (const cap of list) {
    ...
    checks.push(check({
      ...
      domain,
      title: `${id} (${cls.tooling_kind}) is readiness-required under DEC-033 and must be live-qualified`,
      status: STATUS.UNVERIFIED,
      criticality: CRITICALITY.MANDATORY,
      readiness_class: 'CORE_DEVELOPMENT_REQUIRED',
      evidence: { ...base, owner_required_ref: 'DEC-033' },
      gate: `AUTH-GATE-GOOGLE-${String(id).replace(/^[A-Z]+-/, '')}-001`,
      remediation: 'complete required provider authentication/configuration and run node agent-system/orchestration/google-capability-cli.mjs qualify <provider>',
    }));
  }
