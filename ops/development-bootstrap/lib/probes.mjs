...
// User-local install prefixes that vendor installers use (Claude Code, Hermes, npm --prefix). They are
// consulted only AFTER PATH so a binary found here is reported with path_source=USER_LOCAL_FALLBACK and
// the certification raises a PATH-hygiene finding instead of silently widening PATH (closure item 10).
export function userLocalBinDirs(home = os.homedir()) {
  return [path.join(home, '.local', 'bin'), path.join(home, '.npm-global', 'bin'), '/usr/local/bin'];
}

export function requiredServicePath(home = os.homedir()) {
  return [path.join(home, '.local', 'bin'), path.join(home, '.npm-global', 'bin'), '/usr/local/bin', '/usr/bin', '/bin'].join(':');
}

export function locateBinary(binary, { dirs = userLocalBinDirs() } = {}) {
  if (!/^[A-Za-z0-9._-]+$/.test(String(binary || ''))) return { location: null, path_source: null };
  const r = run('sh', ['-c', `command -v ${binary}`], { timeoutMs: 5000 });
  if (r.ok && r.output) return { location: r.output.split('\n')[0], path_source: 'PATH' };
  for (const dir of dirs) {
    const candidate = path.join(dir, binary);
    try { fs.accessSync(candidate, fs.constants.X_OK); return { location: candidate, path_source: 'USER_LOCAL_FALLBACK', fallback_dir: dir }; } catch {}
  }
  return { location: null, path_source: null };
}

export function which(binary) { return locateBinary(binary).location; }

export function parseSemver(text) {
  const m = String(text || '').match(/(?:^|[^0-9])(\d+)\.(\d+)(?:\.(\d+))?/);
  return m ? { major: +m[1], minor: +m[2], patch: +(m[3] || 0), text: `${m[1]}.${m[2]}.${m[3] || 0}` } : null;
}

// Parse the named product rather than the first incidental version in output. OpenSSH, for example,
// commonly prints an OpenSSL version on the same line.
export function parseToolVersion(binary, text) {
  const value = String(text || '');
  let m;
  if (binary === 'ssh') {
    m = value.match(/OpenSSH[_ ](\d+)\.(\d+)(?:p(\d+))?/i);
    return m ? { major: +m[1], minor: +m[2], patch: +(m[3] || 0), text: `${m[1]}.${m[2]}.${m[3] || 0}` } : null;
  }
  if (binary === 'jq') {
...
export function versionProbe(binary, args = ['--version'], { minimum = null, exact = null } = {}) {
  const located = locateBinary(binary);
  if (!located.location) return { installed: false, location: null, path_source: null, version: null, satisfies: false, command: `${binary} ${args.join(' ')}` };
  // Execute the located file, never a bare name, so the fallback path cannot be shadowed by PATH edits.
  const r = run(located.location, args, { timeoutMs: 15000 });
  const version = parseToolVersion(binary, r.output);
  ...
  return { installed: true, location: located.location, path_source: located.path_source, fallback_dir: located.fallback_dir || null, version: version?.text || null, raw: r.output.split('\n')[0], satisfies, command: r.command, ok: r.ok };
}
